...
size_t cubemx_transport_write(struct uxrCustomTransport *transport, const uint8_t *buf,
...
void StartMicroRosTask(void *argument)
{
  ...
  ret = rclc_node_init_default(&node, "stm32f407_node", "", &support);
  if (ret != RCL_RET_OK)
  {
    for (;;)
    {
  ...
  for(;;)
  {
    (void)rcl_publish(&publisher, &message, NULL);
    message.data++;
    osDelay(1000);
  }
